...
data_public = intialize_public_place(R, P, N, Num_public,Hospital_capacity)
...
simulation_points = function(N, R, P, Times){
  ...
  v[v < 0] = 0
  # Step 5
  infectious_ability = rep(1, N)
  protection_ability = rnorm(N, 1, 0.3)
  protection_ability[protection_ability < 0.05] = 0.05
  protection_ability[protection_ability > 1] = 1
  # Step 7
  data_public = intialize_public_place(R, P, N, Num_public,Hospital_capacity)
  # Step 8
  people_duration = data.frame(place = rep(0, N), place_index = rep(0, N), duration = rep(0, N))
  before_place_info = data.frame(index = NULL, X = NULL, Y = NULL, v = NULL, condition = NULL)
  # Step 10
  before_hos_info = data.frame(index = NULL, X = NULL, Y = NULL, v = NULL, condition = NULL)
  ...
  for(t in 1:Times){
    ...
    people_duration$duration = people_duration$duration + 1
    # Step 3
    new_data = random_walk(data, v)
    # Step 4
    ...
    if(t == 35)
      trigger_quarantine = FALSE
    condition = infection(new_data, v, condition, infectious_ability, protection_ability, pc, trigger_social_distance, social_distance)
    # Step 6
    ...
    if(trigger_restaurant == TRUE) #Step 13
      ...
      temp = close_restaurant(new_data, data_public, people_duration, before_place_info, v, condition)
    new_data = temp[[1]]
    before_place_info = temp[[2]]
    v = temp[[3]] 
    data_public = temp[[4]]
    people_duration = temp[[5]]
    # Step 10
    temp = moveto_hospital(data, new_data, data_public, people_duration, before_hos_info, v, condition)
    new_data = temp[[1]]
    before_hos_info = temp[[4]]
    v = temp[[5]]
    data_public = temp[[2]]
    people_duration = temp[[3]]
    # Step 11
    condition = symptom_hos_change(condition, people_duration)
    # Step 12
    temp = moveout_hospital(data, new_data, data_public, people_duration, before_hos_info, v, condition)
    new_data = temp[[1]]
    before_hos_info = temp[[4]]
    v = temp[[5]]
    data_public = temp[[2]]
    ...
    if(trigger_quarantine==T){
      temp = start_Quarantine(v,v_initial,people_duration,Quarantine_ratio = Quarantine_ratio ,condition=condition$condition)
      v=temp[[1]]
      last_quarantine=temp[[2]]
    }
    if(trigger_quarantine==F&last_quarantine==1){
      temp = end_Quarantine(v,v_initial,people_duration,condition=condition$condition)
      v=temp[[1]]
      last_quarantine=temp[[2]]
    }
    # Step 14
    temp=close_contacts(data, new_data,v,v_initial,people_duration,condition=condition$condition)
    v=temp[[1]]
    people_duration=temp[[2]]
    
    temp=close_contacts_end14(data, new_data,v,v_initial,people_duration,condition=condition$condition)
    v=temp[[1]]
    people_duration=temp[[2]]
    
    protection_ability=protection_ability_quarantine(v,people_duration,protection_ability)
    
    # Step 18
    condition = recrudesce(condition, vacday > vacstart)
    # Step 19
    if(t > 20){
      vacday = vacday + 1
      if(vacday > vacstart)
        protection_ability = vaccine(condition, protection_ability)
    }
...
```{r}
temp= simulation_points(N, R, P, Times)
whole_data = temp[[1]]
data_public = temp[[2]]
```
