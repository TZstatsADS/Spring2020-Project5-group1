...
```{r}
infection = function(data, v, condition, infectious_ability){
  ...
  infection_change = function(condition2,infectious_ability2, pc){
    
    infection_radius = v[infection_index] * infection_speed[infection_index] * infectious_ability2[infection_index]
  ...
  condition = infection_change(condition, infectious_ability, pc)
  condition = infection_change(condition, infectious_ability*2, pc/4)
...
intialize_public_place = function(R, P, N, Num_public,Hospital_capacity){
  ...
  U = runif(1, 0, 1)
  data_station = random_public_place(R[L]*U, 2, N/100)
  data_public = rbind(data_public, data_station)
  
  num_hospital = ceiling((Num_public-1)/5)
  num_school = Num_public - 1 - num_hospital
  
  build_public = function(class2, capacity2, num2){
    
    data_public2 = data.frame(X = NULL, Y = NULL, Class = NULL, Capacity = NULL)
    
    capacity_hospital_num = capacity2/num2
    capacity_hospital_num = rnorm(num2, capacity_hospital_num, capacity_hospital_num/3)
    capacity_hospital_num[capacity_hospital_num<1] = 1
    capacity_hospital_num = round(capacity_hospital_num)
    
    U = runif(num2, 0 ,1)
    pos_hospital = sapply(U, function(x) mean(x>density_cumsum)*(L+1))
    
    U = runif(num2, 0 ,1)
    for(i in 1:num2){
      R0 = c(R, 0)
      R_i = (R0[pos_hospital[i]] - R0[pos_hospital[i]+1])*U[i] + R0[pos_hospital[i]+1]
      data_hospital = random_public_place(R_i, class2, capacity_hospital_num[i])
      data_public2 = rbind(data_public2, data_hospital)
    }
    return(data_public2)
  }
...
plot_points = function(data, condition, data_public){
  ...
  data_public$Class = factor(data_public$Class, levels = 1:3, labels = c('Hospital', 'Station', 'Restaurant'))
  
  t = tibble(X = data$X, Y = data$Y, condition = factor(condition, levels = 1:7))
  g = t %>%
    ggplot(aes(x = X, y = Y, color = condition)) + 
    geom_point(size = 0.1, alpha = 0.6) + 
    geom_point(data = data_public, aes(x = X, y = Y, size = log(Capacity)/4, color = Class), alpha = 0.8) + 
    scale_color_manual(values = c('1' = '#00CC00', '2' = '#FFCC00', '3' = '#FF69B4', '4' = '#DC143C', '5' = '#8B0000', '6' = '#000000', '7' = '#00CCFF', 'Hospital' = '#FF0000', 'Station' = '#FF6600', 'Restaurant' = '#7FFFD4'), labels = c('Healthy', 'Incubation', 'Moderate', 'Severe', 'Cirtical', 'Death', 'Cure', 'Hospital', 'Station', 'Restaurant'), name = 'Condition') +
...
## Step 8: Close to public areas

## Step 9: Hospital is working!

## Step 10: We have doctor

## Step 11: Train station bring virus

## Step 12: Temporary hospital and doctors
...
simulation_points = function(N, R, P, Times){
  ...
  for(t in 1:Times){
    ...
    # Step 5
    condition = infection(new_data, v, condition, infectious_ability)
    # Step 6
